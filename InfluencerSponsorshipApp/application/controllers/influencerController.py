...
def influencerMessages():
    ...
    return render_template("influencer/messages.html",all=all,read=read,unread=unread,sent = sent,username = session["username"])
...
def sendIMessage():
    ...
    return render_template("/influencer/send_message.html",username=session["username"])
...
def influencerreply(id,reciever):
    ...
    if request.method=="POST":
        ...
        return render_template("influencer/reply_message.html",username=session["username"])
...
def influencerDashboard():
    ...
    return render_template("influencer/dashboard.html",username=session["username"],new = row[0])
...
def influencerSettings():
    ...
    if request.method == "POST":
        ...
        with db.engine.begin() as connection:
            ...
            return render_template("influencer/settings.html",username=session["username"],userDetails = userDetails,new = row[0],category=categorys,method="GET")
    

    return render_template("influencer/settings.html",userDetails = userDetails[0],new = row[0],username = session["username"],category=categorys)
...
def seeCampaignsI():
    ...
    with db.engine.begin() as connection:
        ...
        query = text("SELECT * FROM campaign WHERE visibility = 'public' ORDER BY name")
        results = connection.execute(query)
        all = results.fetchall()
    ...
    if request.method =="POST":
        ...
        try:
            ...
            try:
                with db.engine.begin() as connection:
                    query = text("SELECT * FROM campaign WHERE (name = :name OR name LIKE :show) AND visibility = 'public' ORDER BY name")
                    ...
                    return render_template("influencer/campaigns.html",campaigns=rows,username=session["username"],new=row[0],method="GET")
            except Exception as e:
                flash(f"Error occurred: {e}")
                return render_template("influencer/campigns.html",campaigns=all,username=session["username"],new=row[0],method="GET")
            ...
            try:
                ...
                try:
                    with db.engine.begin() as connection:
                        query = text(f"SELECT * FROM campaign WHERE (niche = :n OR niche LIKE :show) AND visibility = 'public' ORDER BY name")
                ...
                try:
                    if filterbychoice =="isflagged":
                        if filterbyvalue == "not flagged":
                            ...
                            filterbyvalue = 1
                        with db.engine.begin() as connection:
                            query = text("SELECT * FROM campaign WHERE isflagged = :filterbyvalue AND visibility = 'public' ORDER BY name")
                        ...
                        if filterbyvalue == "on going":
                            query = text(f"SELECT * FROM campaign WHERE end_date > '{date}' AND start_date < '{date}' AND visibility = 'public'  ORDER BY name")
                        
                        elif filterbyvalue == "scheduled":
                            query = text(f"SELECT * FROM campaign WHERE start_date > '{date}' AND visibility = 'public' ORDER BY name")
                        
                        else:
                            query = text(f"SELECT * FROM campaign WHERE end_date < '{date}' AND visibility = 'public' ORDER BY name")
                        with db.engine.begin() as connection:
                        ...
                        with db.engine.begin() as connection:
                            query = text("SELECT * FROM campaign WHERE category = :filterbyvalue AND visibility = 'public' ORDER BY name")
                            details = {"filterbyvalue" :filterbyvalue}
                            result = connection.execute(query,details)
                            rows = result.fetchall()
    ...
    return render_template("influencer/campaigns.html",username=session["username"],new=row[0],campaigns = all)